...
class ResPartner(models.Model):
	...
	def _get_accounting_partner_party_values(self):
		...
		if self.with_company(self.company_id or 1).property_account_position_id:
			if (not self.with_company(self.company_id or 1).property_account_position_id.tax_level_code_id
					or not self.with_company(self.company_id or 1).property_account_position_id.tax_scheme_id
					or not self.with_company(self.company_id or 1).property_account_position_id.listname):
				raise UserError(msg8 % self.name)

			tax_level_codes = ''
			tax_scheme_code = self.with_company(self.company_id or 1).property_account_position_id.tax_scheme_id.code
			tax_scheme_name = self.with_company(self.company_id or 1).property_account_position_id.tax_scheme_id.name
		else:
			raise UserError(msg9 % self.name)


		for tax_level_code_id in self.with_company(self.company_id or 1).property_account_position_id.tax_level_code_id:
			if tax_level_codes == '':
		...
		return {
			...
			'listName': self.with_company(self.company_id or 1).property_account_position_id.listname,
			'TaxLevelCode': tax_level_codes,
			# self.with_company(self.company_id).property_account_position_id.tax_level_code_id.code,
			'TaxSchemeID': self.with_company(self.company_id or 1).property_account_position_id.tax_scheme_id.code,
			'TaxSchemeName': self.with_company(self.company_id or 1).property_account_position_id.tax_scheme_id.name,
