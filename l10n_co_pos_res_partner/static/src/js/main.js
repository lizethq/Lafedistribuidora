...
screens.ClientListScreenWidget.include({
    ...
    save_client_details: function(partner) {
        ...
        var chack_digit = $(".client-check-digit").val();
        var doctype = $(".client-doctype").val()


        if(doctype == '') {
                this.gui.show_popup('error','Debe seleccionar tipo de documento !!');
                return;
        }
        if(doctype == '6') {
            if(chack_digit == '') {
                this.gui.show_popup('error','Debe agregar el digito de verificacion !!');
                return;
            }
        }
        if(zip_id == '') {
                this.gui.show_popup('error','Debe seleccionar una ubicacion !!');
                return;
        }
        if(property_account_position_id == '') {
                this.gui.show_popup('error','Debe seleccionar una posicion fiscal !!');
                return;
        }
        if(!is_company) {
            if(!first_name || !first_lastname) {
                this.gui.show_popup('error','Primer Nombre y Primer Apellido son requeridos!!');
                return;
            }
